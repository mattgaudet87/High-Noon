...
const SKIN = 0xd9a066;
const STICK = 0xc0392b;
const FUSE = 0xeadbc4;

// Powder Man deals splash damage instead of hitting a single target.
// Drawn with a bandolier of dynamite sticks so the threat reads clearly.
export function drawPowderman(
  ...
  // Bandolier of dynamite sticks worn across the chest
  graphics.fillStyle(0x5c4326, 1);
  graphics.fillRect(x - 11, y - 40, 22, 4);
  for (let i = -1; i <= 1; i++) {
    graphics.fillStyle(STICK, 1);
    graphics.fillRect(x + i * 7 - 2, y - 39, 4, 12);
    graphics.fillStyle(FUSE, 1);
    graphics.fillRect(x + i * 7 - 1, y - 41, 2, 2);
  }
  ...
  // Lit stick of dynamite held out, ready to throw
  graphics.fillStyle(SKIN, 1);
  graphics.fillCircle(x + facing * 16, y - 32, 5);
  graphics.fillStyle(STICK, 1);
  graphics.fillRect(x + facing * 18, y - 38, 5, 14);
  graphics.fillStyle(0xf5d76e, 0.9);
  graphics.fillCircle(x + facing * 20, y - 40, 2.5);
